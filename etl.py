...
def process_log_file(cur, filepath):
    ...
    for index, row in df.iterrows():
        ...
        strttime = pd.to_datetime(row.ts,  unit='ms')
        if strttime is not None and row.userId is not None:
            songplay_data = list([strttime,row.userId, row.level, songid, artistid, row.sessionId, row.location, row.userAgent])
            cur.execute(songplay_table_insert, songplay_data)


def process_data(cur, conn, filepath, func):
    """
    Description: This function can be used to call the functions to process song/log files 
    ...
    Arguments:
        ...
        filepath: log data file path
        func: function to be called to process the files in filepath. 
